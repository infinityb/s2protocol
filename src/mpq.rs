...
pub fn read_header<R: Read>(rdr: &mut R) -> io::Result<MpqHeader> {
...
impl<R> MpqArchive<R> where R: Read+Seek {
    ...
    pub fn read_hash_table(&mut self) -> io::Result<Vec<MpqHashTableEntry>> {
    ...
    pub fn read_block_table(&mut self) -> io::Result<Vec<MpqBlockTableEntry>> {
